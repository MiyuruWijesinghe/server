...
public interface UserDao extends JpaRepository<User, Integer>, CriteriaQuerySupplement<User> {
    ...
    List<User> findUsersByEmployee(@Param("id") Integer id);

    @Query("select new Employee(em.id, em.code, em.callingname, em.nametitle) from Employee em where EXISTS (select u from User u where u.employee.id = em.id)")
    List<Employee> findAllUserEmployees();

    @Query("select new Employee(em.id, em.code, em.callingname, em.nametitle) from Employee em where NOT EXISTS (select u from User u where u.employee.id = em.id)")
